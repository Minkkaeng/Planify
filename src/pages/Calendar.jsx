...
function Calendar() {
  ...
  const LONG_PRESS_MS = 800; // 0.8초 정도 -> '꾹' 느낌
  ...
  const selectedTasks = useMemo(() => {
    return (tasksByDate.get(selectedDate) || []).sort((a, b) => b.id - a.id);
            ...
            <ul className="calendar-task-list">
              {selectedTasks.map((t) => (
                <li key={t.id} className={"calendar-task-item" + (t.done ? " done" : "")}>
                  ...
                  <span className="meta">
                    {PROJECT_LABEL[t.project] || "기타"} ·{" "}
                    {t.priority === "high" ? "우선순위 높음" : t.priority === "low" ? "우선순위 낮음" : "우선순위 보통"}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="cal-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="cal-add-form" onSubmit={addTaskToModalDate}>
              <input
                type="text"
                value={newTask.title}
                onChange={(e) => changeNewTask("title", e.target.value)}
                placeholder="할 일 제목"
              />
              ...
              <textarea
                rows={2}
                value={newTask.memo}
                onChange={(e) => changeNewTask("memo", e.target.value)}
                placeholder="메모 (선택)"
              />
